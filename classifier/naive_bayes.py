...
def predict_category(document):
    ...
    # データベースから訓練データと学習データを抽出
    df1 = pd.read_sql_query('select text,category from classifier_article_data limit 160', conn)
    df2 = pd.read_sql_query('select text from classifier_article_data limit 159,160', conn)
